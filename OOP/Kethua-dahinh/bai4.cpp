...
class gv : public person{
	...
	public:
		gv() : person() {}	
		gv(string ma, string name, string ns, string address, string khoa, ll luong, string lop) : person(ma, name, ns, address) {
			this->khoa = khoa;
			this->luong = luong;
			this->lop = lop;
		}
...
void ex(string HOC = "A year begins in the spring. A young life begins.") {
	...
	while (TC --> 0) {
		int n; cin >> n;
		cin.ignore();
		map<string, vector<gv>> a;
		map<string, vector<sv>> b;
		for(int i = 0; i < n; i++){
			string ma, name, ns, address;
			getline(cin, ma);
			getline(cin, name);
			...
			if(ma.substr(0, 2) == "GV"){
				...
				ll luong; cin >> luong; cin.ignore();
				string lop; getline(cin, lop);
				gv tmp(ma, name, ns, address, khoa, luong, lop);
				tmp.chuanhoa();
				a[tmp.getAddress()].pb(tmp);
			}
			else{
				string lop; cin >> lop;
				double gpa; cin >> gpa;cin.ignore();
				sv k(ma, name, ns, address, lop, gpa);
				k.chuanhoa();
				b[k.getAddress()].push_back(k);
			}
		}
		string s; 
		getline(cin, s);
		cout << "DANH SACH GIAO VIEN CO DIA CHI TAI " << s <<" :\n";
		for(auto it : a[s])
			it.toString();
			
		cout << "DANH SACH SINH VIEN CO DIA CHI TAI " << s << " :\n";
		for(auto it : b[s])
			it.toString();	
	}
}
