...
// DANH SACH SINH VIEN THEO NGANH KE TOAN:
...
void ex(){
    ...
    while(q--){
        ...
        getline(cin, s);
        vietHoa(s);
        cout << "DANH SACH SINH VIEN THEO NGANH " << s << ":\n";
        string major = convert(s);
        if(major == "CN" or major == "AT"){
            for(auto it : mp[major]){
                if(it.getMa().find("E") == string :: npos){
                    cout  << it << "\n";
                }
            }
        }
